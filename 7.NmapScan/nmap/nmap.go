...
type Scanner struct {
	...
	args       []string  		// 这里存储扫描的网址 也存储扫描端口？（中间一个-p间隔着）
	binaryPath string    		// 存储命令行
	ctx        context.Context  // 存储超时
...
// NewScanner创建一个新的扫描器，并且可以采用一些选项来应用于扫描器。
// options ... 输入参数可变化，具体参考 https://blog.csdn.net/Lazyboy_/article/details/103289750
...
func (s *Scanner) Run() (result *Run, warnings []string, err error) {
	...
	// 当扫描完成时，做一个goroutine通知选择  cmd.Start 与 cmd.Wait 必须一起使用  cmd.Wait 等待命令结束
